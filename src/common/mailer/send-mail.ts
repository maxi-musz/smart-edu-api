...
export const sendOnboardingMailToSchoolOwner = async (
  ...
  try {
    ...
    console.log(
      colors.green(`Onboarding email sent to ${payload.school_email}`),
    );
  } catch (error) {
    console.log(colors.red('Error sending onboarding email: '), error);
    throw ResponseHelper.error('Error sending onboarding email', error.message);
  }
};
...
export const sendOnboardingMailToBTechAdmin = async (
  ...
  try {
    ...
    const emailProvider = EmailProviderFactory.getProvider();
    const htmlContent = onboardingSchoolAdminNotificationTemplate({
      ...payload,
    });
    const adminEmail =
      process.env.BTECH_ADMIN_EMAIL || 'besttechnologies25@gmail.com';
    ...
    console.log(
      colors.green(`New school Onboarding email sent to ${adminEmail}`),
    );
  } catch (error) {
...
export const sendPasswordResetOtp = async ({
  email,
  otp,
}: SendResetOtpProps): Promise<void> => {
  const emailProvider = EmailProviderFactory.getProvider();
...
export const sendLoginOtpByMail = async ({
  email,
  otp,
}: SendResetOtpProps): Promise<void> => {
