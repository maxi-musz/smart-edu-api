...
export class CloudinaryService {
  ...
  async uploadToCloudinary(
    ...
    try {
      ...
      console.log(
        colors.green('All files successfully uploaded to cloudinary'),
      );
      return successfulUploads;
    } catch (error) {
      console.log(colors.red('Error in file upload process'));
      throw ResponseHelper.error('Failed to upload files', error.message, 400);
    }
  }
  ...
  async deleteFromCloudinary(publicIds: string[]): Promise<any[]> {
    ...
    try {
      ...
      console.log(
        colors.green('All files successfully deleted from cloudinary'),
      );
      return results;
    } catch (error) {
  ...
  async uploadVideoToCloudinary(
    ...
    console.log(
      colors.yellow(`Starting video upload: ${videoFile.originalname}`),
    );
    console.log(
      colors.blue(`File size: ${(videoFile.size / 1024 / 1024).toFixed(2)} MB`),
    );
    ...
    if (!fileExtension || !this.ALLOWED_VIDEO_FORMATS.includes(fileExtension)) {
      ...
      console.log(colors.red(errorMessage));
      throw ResponseHelper.error('Invalid video format', errorMessage, 400);
    }
    ...
    return new Promise<CloudinaryUploadResult>((resolve, reject) => {
      ...
      const timeoutId = setTimeout(() => {
        reject(
          new Error(`Upload timeout after ${timeout / 1000 / 60} minutes`),
        );
      }, timeout);
      ...
      const uploadOptions = {
        ...
        public_id:
          options?.public_id ||
          `${Date.now()}_${videoFile.originalname.split('.')[0]}`,
      ...
      upload.on('progress', (progress) => {
        ...
        const progressPercent = Math.round(
          (uploadedBytes / videoFile.size) * 100,
        );
        console.log(
          colors.blue(
            `Upload progress: ${progressPercent}% (${(uploadedBytes / 1024 / 1024).toFixed(2)} MB / ${(videoFile.size / 1024 / 1024).toFixed(2)} MB)`,
          ),
        );
      });
