...
export class UnifiedJwtGuard extends AuthGuard(['jwt1', 'library-jwt']) {
  ...
  canActivate(context: ExecutionContext) {
    ...
    if (!authHeader) {
      console.log(
        colors.red('Unified JWT Guard - No authorization header found'),
      );
    ...
    if (!authHeader.startsWith('Bearer ')) {
      console.log(
        colors.red('Unified JWT Guard - Invalid authorization header format'),
      );
  ...
  handleRequest(err: any, user: any, info: any, context: ExecutionContext) {
    if (err || !user) {
      console.log(
        colors.red(
          `Unified JWT Guard - Authentication failed: ${info?.message || 'Unknown error'}`,
        ),
      );
    ...
    console.log(
      colors.green(
        `Unified JWT Guard - Authenticated ${userType}: ${user.sub}`,
      ),
    );
