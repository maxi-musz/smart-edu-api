...
export class ExploreService {
  ...
  async getExploreData(user: any) {
    ...
    this.logger.log(
      colors.cyan(`📚 Fetching explore page data for user ${user.email}...`),
    );

    try {
      const accessibleSubjectIds =
        await this.accessControlHelper.getAccessibleSubjectIds(userId);
      ...
      this.logger.log(
        colors.green(
          `✅ Explore data retrieved: ${formattedClasses.length} classes, ${formattedSubjects.length} subjects, ${recentVideos.length} videos`,
        ),
      );
      ...
      this.logger.error(
        colors.red(`❌ Error fetching explore data: ${error.message}`),
      );
      throw error;
    }
  }
  ...
  async getSubjects(user: any, queryDto: QuerySubjectsDto) {
    ...
    this.logger.log(
      colors.cyan(
        `📚 Fetching subjects - Page: ${page}, Limit: ${limit}, ClassId: ${classId || 'all'}, Search: ${search || 'none'}`,
      ),
    );
    ...
    try {
      ...
      this.logger.log(
        colors.green(
          `✅ Subjects retrieved: ${formattedSubjects.length} items (Page ${page}/${totalPages})`,
        ),
      );
      return ResponseHelper.success('Subjects retrieved successfully', data);
    } catch (error) {
      this.logger.error(
        colors.red(`❌ Error fetching subjects: ${error.message}`),
      );
      throw error;
    }
  }
  ...
  async getVideos(user: any, queryDto: QueryVideosDto) {
    ...
    this.logger.log(
      colors.cyan(
        `🎥 Fetching videos - Page: ${page}, SubjectId: ${subjectId || 'all'}, TopicId: ${topicId || 'all'}`,
      ),
    );
    ...
    try {
      ...
      this.logger.log(
        colors.green(
          `✅ Videos retrieved: ${videos.length} items (Page ${page}/${totalPages})`,
        ),
      );
      return ResponseHelper.success('Videos retrieved successfully', data);
    } catch (error) {
      this.logger.error(
        colors.red(`❌ Error fetching videos: ${error.message}`),
      );
      throw error;
    }
  }
  ...
  async getTopicsForSubject(subjectId: string, user: any) {
    ...
    try {
      ...
      this.logger.log(
        colors.yellow(`📖 Subject: ${subject.name} (${subject.code})`),
      );

      // Get excluded resource IDs (library owner may have turned off individual topics/videos/materials/assessments)
      const excluded = await this.accessControlHelper.getExcludedIdsForSubject(
        userId,
        subjectId,
      );
      ...
      const subjectLevelAssessment =
        await this.prisma.libraryAssessment.findFirst({
          where: {
            subjectId: subjectId,
            // topicId: null,
            status: 'ACTIVE',
      ...
      const subjectAssessmentExcluded =
        subjectLevelAssessment &&
        excludedAssessmentIds.has(subjectLevelAssessment.id);
      const libraryAssessmentInfo =
        subjectLevelAssessment && !subjectAssessmentExcluded
          ? {
      ...
      this.logger.log(
        colors.yellow(
          `📑 Found ${topics.length} topics (${allTopics.length - topics.length} excluded by library owner)`,
        ),
      );
        ...
        topics.map(async (topic) => {
          ...
          const assessmentsWithCounts = await Promise.all(
            assessments.map(async (assessment) => {
              const questionCount =
                await this.prisma.libraryAssessmentQuestion.count({
          ...
          if (userId && assessmentsWithCounts.length > 0) {
            ...
            this.logger.log(
              colors.yellow(
                `🔍 Searching submissions for userId: ${userId}, assessmentIds: ${assessmentIds.join(', ')}`,
              ),
            );

            const userAttempts =
              await this.prisma.libraryAssessmentAttempt.findMany({
                where: {
                  userId: userId,
            ...
            this.logger.log(
              colors.yellow(
                `📦 Found ${userAttempts.length} attempts for topic: ${topic.title}`,
              ),
            );
            ...
            this.logger.log(
              colors.gray(
                `⚠️ No submissions check: userId=${userId}, assessmentsCount=${assessmentsWithCounts.length}`,
              ),
            );
          }
          ...
          const visibleVideos = videos.filter(
            (v) => !excludedVideoIds.has(v.id),
          );
          const visibleMaterials = materials.filter(
            (m) => !excludedMaterialIds.has(m.id),
          );
          const visibleAssessmentsWithCounts = assessmentsWithCounts.filter(
            (a) => !excludedAssessmentIds.has(a.id),
          );
          const visibleAssessmentIds = new Set(
            visibleAssessmentsWithCounts.map((a) => a.id),
          );
          const visibleSubmissions = submissions.filter((s) =>
            visibleAssessmentIds.has(s.assessmentId),
          );
      ...
      const subjectStats = {
        ...
        videosCount: topicsWithResources.reduce(
          (sum, topic) => sum + topic.statistics.videosCount,
          0,
        ),
        materialsCount: topicsWithResources.reduce(
          (sum, topic) => sum + topic.statistics.materialsCount,
          0,
        ),
      };
      ...
      const submissionsSummary = userId
        ? `, ${totalSubmissions} submissions`
        : '';

      this.logger.log(
        colors.green(`✅ Complete resources retrieved for "${subject.name}"`),
      );
      this.logger.log(
        colors.cyan(
          `📊 Summary: ${subjectStats.topicsCount} topics, ${subjectStats.videosCount} videos, ${subjectStats.materialsCount} materials${submissionsSummary}`,
        ),
      );

      return ResponseHelper.success(
        'Subject resources retrieved successfully',
        data,
      );
    } catch (error) {
      ...
      this.logger.error(
        colors.red(`❌ Error fetching subject resources: ${error.message}`),
      );
      throw error;
    }
  }
  ...
  async playVideo(user: any, videoId: string) {
    this.logger.log(
      colors.cyan(
        `🎥 User ${user?.sub ?? user?.email} requesting video playback: ${videoId}`,
      ),
    );
    const userId = user?.sub;
    if (!userId) {
    ...
    try {
      ...
      if (!video) {
        this.logger.error(
          colors.red(`❌ Video not found or not published: ${videoId}`),
        );
      ...
      if (!existingView) {
        ...
        this.logger.log(
          colors.green(
            `✅ New unique view recorded: "${video.title}" by user ${userId} (Total: ${updatedViews})`,
          ),
        );
      } else {
        this.logger.log(
      ...
      this.logger.log(
        colors.green(
          `✅ Video playback granted: "${video.title}" (${updatedViews} unique views)`,
        ),
      );
      ...
      this.logger.error(
        colors.red(`❌ Error retrieving video for playback: ${error.message}`),
      );
      throw error;
    }
  }
