...
export class SchoolAccessControlController {
  constructor(
    private readonly accessControlService: SchoolAccessControlService,
  ) {}
  ...
  @ApiOperation({
    ...
    description:
      'View all resources that library owners have granted to your school',
  })
  @ApiResponse({
    status: 200,
  ...
  async getAvailableResources(
    @GetUser() user: any,
    @Query() query: QueryAvailableResourcesDto,
  ) {
  ...
  @ApiOperation({
    ...
    description:
      'School directors and admins can control which users, roles, or classes can access library resources',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Grant the same resource access to multiple users or classes at once',
  })
  @ApiResponse({ status: 201, description: 'Bulk access granted' })
  @ApiResponse({ status: 403, description: 'Forbidden' })
  async grantBulkAccess(
    @GetUser() user: any,
    @Body() dto: SchoolGrantBulkAccessDto,
  ) {
  ...
  @ApiOperation({
    ...
    description:
      'School directors/admins can exclude subjects so non-admin users do not see them in explore. School owner still sees all.',
  })
  @ApiResponse({ status: 201, description: 'Subject excluded successfully' })
  @ApiResponse({
    status: 400,
    description: 'School does not have library access to this subject',
  })
  @ApiResponse({
    status: 403,
    description: 'Forbidden - only school directors and admins',
  })
  @ApiResponse({ status: 404, description: 'Subject not found' })
  async excludeSubject(
    @GetUser() user: any,
    @Body() dto: SchoolExcludeSubjectDto,
  ) {
  ...
  @ApiOperation({
    ...
    description:
      'Remove the school-level subject exclusion so the subject is visible again to non-admin users.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Returns subject IDs that the school owner has excluded. Use this to show correct "Visible to school" toggle state (OFF for these, ON for others).',
  })
  @ApiResponse({
    status: 200,
    description: 'Excluded subjects retrieved successfully',
  })
  @ApiResponse({
    status: 403,
    description: 'Forbidden - only school directors and admins',
  ...
  @ApiOperation({
    ...
    description:
      'Remove all school-level exclusions so teachers and students see all subjects the library granted. Use when toggles should all be ON.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'View all library resources a specific user in your school can access',
  })
