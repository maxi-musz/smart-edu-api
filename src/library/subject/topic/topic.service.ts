  ...
  async getTopicMaterials(
    ...
    try {
      ...
      const totalVideoViews = videos.reduce(
        (sum, video) => sum + ((video.views as number) || 0),
        0,
      );
      const totalVideoDuration = videos.reduce(
        (sum, video) => sum + ((video.durationSeconds as number) || 0),
        0,
      );
      ...
      const statistics = {
        ...
        // CBT analysis
        totalCbtQuestions: cbts.reduce(
          (sum, cbt) => sum + (cbt._count?.questions || 0),
          0,
        ),
        totalCbtAttempts: cbts.reduce(
          (sum, cbt) => sum + (cbt._count?.attempts || 0),
          0,
        ),
