...
export class SubjectService {
  ...
  async createSubject(
    ...
    this.logger.log(
      colors.cyan(
        `[LIBRARY SUBJECT] Creating subject for library user: ${user.email}`,
      ),
    );
    ...
    try {
      ...
      if (payload.code) {
        ...
        if (existingSubject) {
          this.logger.error(
            colors.red(
              `Subject code already exists in platform: ${payload.code}`,
            ),
          );
          throw new BadRequestException(
            `Subject code '${payload.code}' already exists in your platform`,
          );
        }
      }
      ...
      if (thumbnailFile) {
        ...
        if (!allowedMimeTypes.includes(thumbnailFile.mimetype)) {
          this.logger.error(
            colors.red(
              'Invalid thumbnail file type. Allowed types: JPEG, PNG, GIF, WEBP',
            ),
          );
          throw new BadRequestException(
            'Invalid thumbnail file type. Allowed types: JPEG, PNG, GIF, WEBP',
        ...
        try {
          ...
          this.logger.log(
            colors.cyan(`Uploading thumbnail: ${thumbnailFile.originalname}`),
          );
          ...
          this.logger.log(
            colors.green(
              `✅ Thumbnail uploaded successfully: ${uploadResult.url}`,
            ),
          );
        } catch (uploadError: any) {
          this.logger.error(
            colors.red(`❌ Failed to upload thumbnail: ${uploadError.message}`),
          );
          throw new BadRequestException(
      ...
      let subject: any;
      try {
        subject = await this.prisma.$transaction(
          async (tx) => {
        ...
        if (thumbnailUploadSucceeded && thumbnailKey) {
          this.logger.log(
            colors.yellow(`🔄 Attempting to rollback thumbnail upload...`),
          );
          try {
            await this.storageService.deleteFile(thumbnailKey);
            this.logger.log(
              colors.yellow(
                `🗑️ Rolled back: Deleted thumbnail from storage due to database operation failure`,
              ),
            );
          } catch (deleteError: any) {
            this.logger.error(
              colors.red(
                `❌ Failed to rollback thumbnail file: ${deleteError.message}`,
              ),
            );
          }
        }
  ...
  async updateSubject(
    ...
    this.logger.log(
      colors.cyan(
        `[LIBRARY SUBJECT] Updating subject: ${subjectId} for library user: ${user.email}`,
      ),
    );
    ...
    try {
      ...
      this.logger.log(
        colors.green(`Subject updated successfully: ${subject.id}`),
      );
      ...
      this.logger.error(
        colors.red(`Error updating subject: ${error.message}`),
        error.stack,
      );
  ...
  async updateSubjectThumbnail(
    ...
    this.logger.log(
      colors.cyan(
        `[LIBRARY SUBJECT] Updating thumbnail for subject: ${subjectId} for library user: ${user.email}`,
      ),
    );
    ...
    try {
      ...
      try {
        ...
        this.logger.log(
          colors.cyan(`Uploading new thumbnail: ${thumbnailFile.originalname}`),
        );
        const uploadResult = await this.storageService.uploadFile(
          thumbnailFile,
          folder,
        ...
        this.logger.log(
          colors.green(
            `✅ New thumbnail uploaded successfully: ${uploadResult.url}`,
          ),
        );
      } catch (uploadError: any) {
        this.logger.error(
      ...
      try {
        ...
        if (oldThumbnailKey) {
          try {
            this.logger.log(
              colors.cyan(
                `Deleting old thumbnail from storage: ${oldThumbnailKey}`,
              ),
            );
            await this.storageService.deleteFile(oldThumbnailKey);
            this.logger.log(
              colors.green(`✅ Old thumbnail deleted successfully`),
            );
          } catch (deleteError: any) {
            // Log error but don't fail the operation
            this.logger.error(
              colors.yellow(
                `⚠️ Failed to delete old thumbnail: ${deleteError.message}`,
              ),
            );
          }
        }
        ...
        this.logger.log(
          colors.green(`Subject thumbnail updated successfully: ${subject.id}`),
        );
        return new ApiResponse(
          true,
          'Subject thumbnail updated successfully',
          subject,
        );
      } catch (dbError: any) {
        ...
        if (thumbnailUploadSucceeded && newThumbnailKey) {
          ...
          try {
            ...
            this.logger.log(
              colors.yellow(
                `🗑️ Rolled back: Deleted new thumbnail from storage due to database operation failure`,
              ),
            );
          } catch (deleteError: any) {
            this.logger.error(
      ...
      this.logger.error(
        colors.red(`Error updating subject thumbnail: ${error.message}`),
        error.stack,
      );
      throw new InternalServerErrorException(
        'Failed to update subject thumbnail',
      );
    }
  }
  ...
  async deleteSubject(user: any, subjectId: string): Promise<ApiResponse<any>> {
    this.logger.log(
      colors.cyan(
        `[LIBRARY SUBJECT] Deleting subject: ${subjectId} for library user: ${user.email}`,
      ),
    );
    ...
    try {
      ...
      for (const video of existingSubject.videos) {
        ...
        if (video.thumbnailS3Key)
          storageKeysToDelete.push(video.thumbnailS3Key);
      ...
      for (const assignment of existingSubject.assignments) {
        if (assignment.attachmentS3Key)
          storageKeysToDelete.push(assignment.attachmentS3Key);
      }
      ...
      for (const assessment of existingSubject.assessments) {
        for (const question of assessment.questions) {
          if (question.imageS3Key)
            storageKeysToDelete.push(question.imageS3Key);
        }
      }
      ...
      const videosCount =
        existingSubject.videos.length +
        existingSubject.topics.reduce((acc, t) => acc + t.videos.length, 0);
      const materialsCount =
        existingSubject.materials.length +
        existingSubject.topics.reduce((acc, t) => acc + t.materials.length, 0);
      const assignmentsCount =
        existingSubject.assignments.length +
        existingSubject.topics.reduce(
          (acc, t) => acc + t.assignments.length,
          0,
        );
      const assessmentsCount =
        existingSubject.assessments.length +
        existingSubject.topics.reduce(
          (acc, t) => acc + t.assessments.length,
          0,
        );
      const linksCount =
        existingSubject.links.length +
        existingSubject.topics.reduce((acc, t) => acc + t.links.length, 0);
      const commentsCount =
        existingSubject.comments.length +
        existingSubject.topics.reduce((acc, t) => acc + t.comments.length, 0);
      const generalMaterialsCount = existingSubject.generalMaterials.length;

      this.logger.log(
        colors.yellow(
          `🗑️ Preparing to delete subject "${existingSubject.name}" with:`,
        ),
      );
      ...
      this.logger.log(
        colors.yellow(`   - ${generalMaterialsCount} general materials`),
      );
      this.logger.log(
        colors.yellow(`   - ${storageKeysToDelete.length} storage files`),
      );
      this.logger.log(
        colors.yellow(`   - ${hlsPrefixesToDelete.length} HLS folders`),
      );
      ...
      await this.prisma.$transaction(
        async (tx) => {
          ...
          await tx.teacherResourceExclusion.deleteMany({
            where: { subjectId },
          });
          ...
          if (topicIds.length > 0) {
            await tx.libraryResourceAccess.deleteMany({
              where: { topicId: { in: topicIds } },
            });
            await tx.schoolResourceAccess.deleteMany({
              where: { topicId: { in: topicIds } },
            });
            await tx.teacherResourceAccess.deleteMany({
              where: { topicId: { in: topicIds } },
            });
          }
          ...
          if (videoIds.length > 0) {
            await tx.libraryVideoWatchHistory.deleteMany({
              where: { videoId: { in: videoIds } },
            });
            await tx.libraryVideoView.deleteMany({
              where: { videoId: { in: videoIds } },
            });
          }
          ...
          const assessmentIds = [
            ...
            ...existingSubject.topics.flatMap((t) =>
              t.assessments.map((a) => a.id),
            ),
          ];
          if (assessmentIds.length > 0) {
            await tx.libraryAssessmentResponse.deleteMany({
              where: { attempt: { assessmentId: { in: assessmentIds } } },
            });
            await tx.libraryAssessmentAttempt.deleteMany({
              where: { assessmentId: { in: assessmentIds } },
            });
            await tx.libraryAssessmentAnalytics.deleteMany({
              where: { assessmentId: { in: assessmentIds } },
            });
          }
          ...
          if (gmIds.length > 0) {
            ...
            await tx.libraryGeneralMaterialChunk.deleteMany({
              where: { materialId: { in: gmIds } },
            });
            await tx.libraryGeneralMaterialProcessing.deleteMany({
              where: { materialId: { in: gmIds } },
            });
            ...
            const chapterIds = existingSubject.generalMaterials.flatMap((gm) =>
              gm.chapters.map((c) => c.id),
            );
            if (chapterIds.length > 0) {
              await tx.libraryGeneralMaterialChapterFile.deleteMany({
                where: { chapterId: { in: chapterIds } },
              });
              await tx.libraryGeneralMaterialChapter.deleteMany({
                where: { id: { in: chapterIds } },
              });
            }

            // Delete general material class associations
            await tx.libraryGeneralMaterialClass.deleteMany({
              where: { materialId: { in: gmIds } },
            });
          }
          ...
          if (topicIds.length > 0) {
            await tx.libraryVideoLesson.deleteMany({
              where: { topicId: { in: topicIds } },
            });
            await tx.libraryMaterial.deleteMany({
              where: { topicId: { in: topicIds } },
            });
            await tx.libraryAssignment.deleteMany({
              where: { topicId: { in: topicIds } },
            });
            await tx.libraryLink.deleteMany({
              where: { topicId: { in: topicIds } },
            });
            await tx.libraryAssessment.deleteMany({
              where: { topicId: { in: topicIds } },
            });
          }
      ...
      for (const key of storageKeysToDelete) {
        try {
          ...
          this.logger.error(
            colors.yellow(
              `⚠️ Failed to delete file: ${key} - ${deleteError.message}`,
            ),
          );
          failedFilesCount++;
        }
      ...
      for (const prefix of hlsPrefixesToDelete) {
        try {
          ...
          this.logger.error(
            colors.yellow(
              `⚠️ Failed to delete HLS folder: ${prefix} - ${deleteError.message}`,
            ),
          );
          failedFilesCount++;
        }
      ...
      this.logger.log(
        colors.green(
          `✅ Storage cleanup: ${deletedFilesCount} files deleted, ${failedFilesCount} failed`,
        ),
      );
      this.logger.log(
        colors.green(
          `Subject "${existingSubject.name}" deleted successfully with all content`,
        ),
      );

      return new ApiResponse(
        true,
        'Subject and all its content deleted successfully',
        {
          id: subjectId,
      ...
      this.logger.error(
        colors.red(`Error deleting subject: ${error.message}`),
        error.stack,
      );
