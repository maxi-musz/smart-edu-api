...
**Single source of truth** for all Exam Body APIs (exam bodies, subjects, years, assessments, questions). Use this document for frontend integration.
...
**Option image URLs:** Each option can include `imageUrl` (string) if you already have a hosted image URL; the backend will persist it. For file uploads, use "Create Question with Image" (5.1.1) with `optionImages` and `imageIndex`.
...
| image | file | No | Image file for the **question** (JPEG, PNG, GIF, WEBP, max 5MB) |
| optionImages | file[] | No | Image files for **options**, matched by index (see Option image mapping) |
...
```json
{
  ...
  "options": [
    { "optionText": "Circle", "order": 1, "isCorrect": false, "imageIndex": 0 },
    { "optionText": "Triangle", "order": 2, "isCorrect": true, "imageIndex": 1 }
  ]
}
```

**Option image mapping:**
- Each uploaded option image is placed in the `optionImages` array in order.
- In `questionData.options`, set `imageIndex` to the **0-based index** of the corresponding file in `optionImages`.
- The backend sets `imageUrl` on the matched option to the uploaded file URL.
    ...
    "options": [
      {
        ...
        "order": 1,
        "isCorrect": false,
        "imageUrl": "https://s3.amazonaws.com/.../option_1.jpg",
        "createdAt": "2026-01-14T10:00:00.000Z"
      },
      {
        "id": "option_b",
        ...
        "order": 2,
        "isCorrect": true,
        "imageUrl": "https://s3.amazonaws.com/.../option_2.jpg",
        "createdAt": "2026-01-14T10:00:00.000Z"
      }
    ],
    "correctAnswers": [
...
**Note:** If question creation fails after any image upload, all uploaded images are automatically deleted from S3 (rollback).
...
**Content-Type:** `application/json` (no files) or `multipart/form-data` (when uploading images)  
**Auth:** Required (Library JWT + Owner/Manager role)

**Request Body (JSON):** Same shape as create; all fields optional. Use `options[].imageUrl` for direct URLs when not uploading files.

**Request Body (Multipart Form-Data):**

| Field | Type | Description |
|-------|------|-------------|
| questionData | string | JSON string of question data (same shape as JSON body below) |
| image | file | New image file for the **question** (replaces existing image) |
| optionImages | file[] | New image files for **options**, matched by `imageIndex` |

**Example JSON / questionData:**
```json
{
  "questionText": "Updated text",
  "points": 3,
  "options": [
    { "optionText": "A", "order": 1, "isCorrect": false, "imageIndex": 0 },
    { "optionText": "B", "order": 2, "isCorrect": true, "imageIndex": 1 }
  ],
  "imageUrl": null
}
```
...
**Image handling:**
- **Question image:** If `image` file is provided, it replaces the existing question image. If `"imageUrl": null` is set, the question image is removed.
- **Option images:** Each file in `optionImages[]` is matched to the option whose `imageIndex` equals the file's index (0-based). The backend sets `imageUrl` on that option to the uploaded URL. You can also set `options[].imageUrl` directly if you already have hosted URLs (no upload).

**Note:** If `options` is provided, it replaces all existing options (and correct answers).

**Success Response (200):** Same structure as create (question, options with `imageUrl` where applicable, correctAnswers).
