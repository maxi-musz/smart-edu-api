...
export class AssessmentDocs {
  static createQuestionWithImage = {
    operation: ApiOperation({
      ...
      description:
        'Upload image and create question atomically. If question creation fails, image is automatically deleted from S3. This prevents orphaned images.',
    }),
      ...
      schema: {
        ...
        properties: {
          ...
          questionData: {
            type: 'string',
            description:
              'JSON string of question data (CreateAssessmentQuestionDto)',
    ...
    response201: ApiResponse({
      status: 201,
      description: 'Question created successfully with image',
    }),
    response400: ApiResponse({
      status: 400,
      description: 'Bad request - Invalid data or image',
    }),
    ...
    response403: ApiResponse({
      status: 403,
      description: 'Forbidden - No access to assessment',
    }),
  };
}
