...
export class ResultsController {
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves a list of sessions/terms, and for a selected session/term (defaults to current), returns every class managed by the teacher with students and their released subject results (as prepared by the director). Supports pagination of students.',
  })
  @ApiQuery({
    name: 'sessionId',
    required: false,
  ...
  @ApiQuery({
    ...
    required: false,
    type: Number,
    description: 'Page number for students pagination (defaults to 1)',
    example: 1,
  })
  @ApiQuery({
    name: 'limit',
    required: false,
    type: Number,
    description: 'Page size for students pagination (defaults to 30)',
    example: 30,
  })
  @ApiResponse({
    status: 200,
  ...
  getResultMainPageData(
    ...
    return this.resultsService.getResultMainPageData(user, {
      sessionId,
      term,
      page: pageNum,
      limit: limitNum,
