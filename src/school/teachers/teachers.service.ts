...
export class TeachersService {
  ...
  async getTeacherProfile(user: User) {
    ...
    try {
      ...
      this.logger.log(
        colors.green(
          `Teacher profile fetched successfully for: ${teacher.email}`,
        ),
      );
  ...
  async getTeacherTimetable(user: User) {
    this.logger.log(
      colors.cyan(`Fetching timetable for teacher: ${user.email}`),
    );
    ...
    try {
      ...
      const formattedTimetable = {
        ...
        schedule: {
          ...
          TUESDAY: this.formatDaySchedule(
            timetableEntries,
            'TUESDAY',
            timeSlots,
          ),
          WEDNESDAY: this.formatDaySchedule(
            timetableEntries,
            'WEDNESDAY',
            timeSlots,
          ),
          THURSDAY: this.formatDaySchedule(
            timetableEntries,
            'THURSDAY',
            timeSlots,
          ),
      ...
      this.logger.error(
        colors.red(`Error fetching teacher timetable: ${error.message}`),
      );
  ...
  async getTeacherDashboard(user: User) {
    this.logger.log(
      colors.cyan(`Fetching teacher dashboard for: ${user.email}`),
    );
    ...
    try {
      ...
      if (!teacher) {
        this.logger.error(
          colors.red(`Teacher not found for user: ${user.email}`),
        );
      ...
      this.logger.log(
        colors.green(
          `✅ Teacher found: ${teacher.first_name} ${teacher.last_name}`,
        ),
      );
      this.logger.log(
        colors.green(
          `✅ Subjects teaching: ${teacher.subjectsTeaching.length}`,
        ),
      );
      ...
      if (!managedClass) {
        this.logger.error(
          colors.red(
            `No class assigned to manage for teacher: ${teacher.first_name} ${teacher.last_name}`,
          ),
        );
      ...
      this.logger.log(
        colors.green(`✅ Managed class found: ${managedClass?.name}`),
      );

      // Calculate student statistics
      const totalStudents = managedClass?.students.length || 0;
      const maleStudents =
        managedClass?.students.filter(
      ...
      this.logger.log(
        colors.green(`✅ Schedule entries found: ${threeDaySchedule.length}`),
      );

      // Format schedule by day
      const formatDaySchedule = (day: DayOfWeek) => {
        const dayEntries = threeDaySchedule.filter(
          (entry) => entry.day_of_week === day,
        );
      ...
      this.logger.log(
        colors.green(
          `✅ Recent notifications found: ${recentNotifications.length}`,
        ),
      );
      ...
      this.logger.log(
        colors.green(
          `✅ Current session found: ${currentSession.academic_year} - ${currentSession.term} term`,
        ),
      );
      ...
      this.logger.log(
        colors.green(
          `Teacher dashboard fetched successfully for: ${user.email}`,
        ),
      );
      ...
      this.logger.error(
        colors.red(`Error fetching teacher dashboard: ${error.message}`),
      );
  ...
  async fetchStudentTabForTeacher(
    ...
    this.logger.log(
      colors.cyan(`Fetching student tab for teacher: ${user.email}`),
    );
    ...
    try {
      ...
      this.logger.log(
        colors.green(`✅ Classes managing: ${teacher.classesManaging.length}`),
      );
      ...
      this.logger.log(
        colors.green(
          `Student tab fetched successfully for teacher: ${user.email}`,
        ),
      );
      ...
      this.logger.error(
        colors.red(`Error fetching student tab for teacher: ${error.message}`),
      );
  ...
  async fetchSchedulesTabForTeacher(user: User) {
    this.logger.log(
      colors.blue(`Fetching schedules tab for teacher: ${user.email}`),
    );
    ...
    try {
      ...
      this.logger.log(
        colors.green(
          `Schedules tab fetched successfully for teacher: ${user.email}`,
        ),
      );
      ...
      this.logger.error(
        colors.red(
          `Error fetching schedules tab for teacher: ${error.message}`,
        ),
      );
  ...
  async fetchSubjectsTabForTeacher(
    ...
    this.logger.log(
      colors.cyan(
        `Fetching subjects dashboard tab for teacher: ${user.email} with filters: ${JSON.stringify(query)}`,
      ),
    );
    ...
    try {
      ...
      if (isDirector) {
        this.logger.log(
          colors.cyan(
            `✅ User is a director - fetching all subjects for school: ${fullUser.school_id}`,
          ),
        );
        ...
        let filteredSubjects = await Promise.all(
          allSubjects.map(async (subject) => {
            ...
            const [topicsCount, videosCount, materialsCount] =
              await Promise.all([
        ...
        return new ApiResponse(
          true,
          'Subjects retrieved successfully for director',
          {
            academicSession,
            stats,
      ...
      this.logger.log(
        colors.yellow(
          `🔍 Searching for teacher with user_id: ${fullUser.id}, school_id: ${fullUser.school_id}`,
        ),
      );
      ...
      if (!teacher) {
        this.logger.error(
          colors.red(
            `❌ Teacher not found for user: ${user.email} with user_id: ${fullUser.id}, school_id: ${fullUser.school_id}`,
          ),
        );
        return new ApiResponse(false, 'Teacher not found', null);
      }
      ...
      let filteredSubjects = await Promise.all(
        teacher.subjectsTeaching.map(async (teacherSubject) => {
          ...
          const [totalVideos, totalMaterials, totalAssignments] =
            await Promise.all([
      ...
      if (query.academic_session_id) {
        ...
        filteredSubjects = filteredSubjects.filter((subject) =>
          subject.timetableEntries.some(
            (entry) => entry.class && entry.class.id, // This would need to be enhanced if we store academic_session_id in timetable entries
          ),
        );
      }
      ...
      const teachingSubjects = this.uniqueById(
        teacher.subjectsTeaching.map((ts) => ({
          id: ts.subject.id,
          name: ts.subject.name,
          code: ts.subject.code,
          color: ts.subject.color,
      ...
      this.logger.log(
        colors.green(
          `Subjects dashboard tab fetched successfully for teacher: ${user.email} - ${paginatedSubjects.length} subjects returned`,
        ),
      );

      return new ApiResponse(
        true,
        'Subjects dashboard tab fetched successfully',
        responseData,
      );
    } catch (error) {
      this.logger.error(
        colors.red(
          `Error fetching subjects dashboard tab for teacher: ${error.message}`,
        ),
        error,
      );
      return new ApiResponse(
        false,
        `Failed to fetch subjects dashboard tab: ${error.message}`,
        null,
      );
    }
  }
