...
export class DocumentChunkingService {
  ...
  async chunkDocument(
    ...
    this.logger.log(
      colors.blue(`✂️ Starting document chunking for material: ${materialId}`),
    );
    ...
    try {
      ...
      const averageChunkSize =
        chunks.length > 0 ? totalTokens / chunks.length : 0;
      ...
      this.logger.log(
        colors.blue(
          `   - Average chunk size: ${Math.round(averageChunkSize)} tokens`,
        ),
      );
      ...
      this.logger.error(
        colors.red(`❌ Error chunking document: ${error.message}`),
      );
  ...
  private createChunks(
    ...
    for (let i = 0; i < sentences.length; i++) {
      ...
      if (
        currentTokens + sentenceTokens > this.MAX_CHUNK_SIZE &&
      ...
      if (
        currentTokens >= this.CHUNK_SIZE ||
        currentTokens > this.MAX_CHUNK_SIZE
      ) {
        chunks.push(
          this.createChunkFromSentences(
            currentChunk,
  ...
  private determineChunkType(content: string): DocumentChunk['chunkType'] {
    ...
    if (
      trimmed.length < 100 &&
      (trimmed.toUpperCase() === trimmed || /^\d+\.?\s/.test(trimmed))
    ) {
      return 'heading';
    }
    ...
    if (
      lines.length > 2 &&
      lines.every((line) => line.includes('|') || /\s{3,}/.test(line))
    ) {
      return 'table';
    }
    ...
    if (
      trimmed.length < 200 &&
      (trimmed.toLowerCase().includes('figure') ||
  ...
  validateChunks(chunks: DocumentChunk[]): {
    ...
    const smallChunks = chunks.filter(
      (chunk) => chunk.tokenCount < this.MIN_CHUNK_SIZE,
    );
    if (smallChunks.length > 0) {
      issues.push(
        `${smallChunks.length} chunks are too small (less than ${this.MIN_CHUNK_SIZE} tokens)`,
      );
    }

    // Check for chunks that are too large
    const largeChunks = chunks.filter(
      (chunk) => chunk.tokenCount > this.MAX_CHUNK_SIZE,
    );
    if (largeChunks.length > 0) {
      issues.push(
        `${largeChunks.length} chunks are too large (more than ${this.MAX_CHUNK_SIZE} tokens)`,
      );
    }
