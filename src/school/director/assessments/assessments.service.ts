...
export class DirectorAssessmentsService {
  ...
  async getAllAssessments(
    ...
    try {
      this.logger.log(
        colors.cyan(`Getting all assessments for director: ${userId}`),
      );
      ...
      this.logger.log(
        colors.green(`Found ${allAssessments.length} assessments for school`),
      );
      return ResponseHelper.success('Assessments retrieved successfully', {
        assessments: groupedAssessments,
        counts,
      ...
      return ResponseHelper.error(
        `Failed to retrieve assessments: ${error.message}`,
        null,
        500,
      );
    }
