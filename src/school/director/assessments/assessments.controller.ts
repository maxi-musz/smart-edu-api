...
export class DirectorAssessmentsController {
  constructor(
    private readonly assessmentsService: DirectorAssessmentsService,
  ) {}
  ...
  async getAssessmentDashboard(
    ...
    return this.assessmentsService.getAssessmentDashboard(user.sub, {
      page,
      limit,
      status,
      assessmentType,
  ...
  @Get('')
  @ApiOperation({
    summary: 'Get all assessments created by all teachers in the school',
  })
  @ApiQuery({
    name: 'status',
    required: false,
  ...
  async getAllAssessments(
    ...
    return this.assessmentsService.getAllAssessments(user.sub, {
      status,
      subjectId,
      topicId,
      assessmentType,
  ...
  @ApiOperation({
    summary: 'Get assessment details with student attempts for director',
  })
  ...
  async getAssessmentAttempts(
    ...
    return this.assessmentsService.getAssessmentAttempts(
      assessmentId,
      user.sub,
    );
  }
  ...
  @ApiParam({
    name: 'studentId',
    description: 'ID of the student (Student record id)',
  })
  @ApiResponse({
    status: 200,
  ...
  async getStudentSubmission(
    ...
    return this.assessmentsService.getStudentSubmission(
      assessmentId,
      studentId,
      user.sub,
    );
