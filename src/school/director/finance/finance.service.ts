...
export class FinanceService {
  ...
  async fetchFinanceDashboard(
    ...
    const formattedSchoolFees = schoolFees.map((fee) => ({
      ...
      status:
        fee.transaction_type === TransactionType.credit ? 'paid' : 'refunded',
    }));
      ...
      'Finance dashboard data fetched successfully',
      {
        pagination: {
          ...
          total_pages: Math.ceil(
            (schoolFees.length + otherFees.length + expenses.length) / limit,
          ),
        },
        basic_details: {
          ...
          net_balance:
            (finance?.total_revenue || 0) - (finance?.amount_withdrawn || 0),
        },
        transactions: {
