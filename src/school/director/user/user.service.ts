...
import {
  ...
  StudentInfoDto,
  StudentAddressDto,
  StudentClassDto as MobileStudentClassDto,
  CurrentSessionDto,
  AcademicInfoDto,
  SubjectEnrolledDto,
...
export class UserService {
  ...
  async getUserProfile(user: User) {
    ...
    try {
      ...
      this.logger.log(
        colors.green(`User profile retrieved successfully for: ${user.email}`),
      );
  ...
  async getStudentProfile(user: User) {
    ...
    try {
      ...
      if (!userProfile.student) {
        this.logger.error(
          colors.red(`Student record not found for user: ${user.email}`),
        );
      ...
      const performanceData = await this.getStudentPerformanceData(
        userProfile.id,
        userProfile.student.current_class_id,
      );
      ...
      const academicInfo: StudentAcademicInfoDto = {
        ...
        dateOfBirth: userProfile.student.date_of_birth
          ? formatDate(userProfile.student.date_of_birth)
          : undefined,
        ...
        currentClass: {
          ...
          classTeacherId:
            userProfile.student.current_class?.classTeacherId || '',
      ...
      const studentProfile: StudentProfileDto = {
        ...
        displayPicture:
          typeof userProfile.display_picture === 'string'
      ...
      this.logger.log(
        colors.green(
          `Student profile retrieved successfully for: ${user.email}`,
        ),
      );
      ...
      this.logger.error(
        colors.red(`Error fetching student profile: ${error.message}`),
      );
    ...
    try {
      ...
      try {
        const studentPerformance =
          await this.prisma.studentPerformance.findFirst({
            where: {
              student_id: userId,
              class_id: classId,
            },
        ...
        this.logger.warn(
          colors.yellow(`Could not fetch class position: ${error.message}`),
        );
      }
      ...
      this.logger.warn(
        colors.yellow(`Could not fetch performance data: ${error.message}`),
      );
      return {
        averageScore: 0,
        totalAssessments: 0,
  ...
  async getMobileStudentProfile(user: User) {
    this.logger.log(
      colors.cyan(`Fetching mobile student profile for: ${user.email}`),
    );
    ...
    try {
      ...
      const subjectsEnrolled = await this.getStudentSubjects(
        userProfile.student.current_class_id,
        userProfile.school_id,
        userProfile.student.academic_session_id,
      );
      ...
      const attendancePercentage = await this.getStudentAttendancePercentage(
        userProfile.id,
        userProfile.student.current_class_id,
      );

      // Get student achievements
      const recentAchievements = await this.getStudentAchievements(
        userProfile.id,
        userProfile.student.academic_session_id,
      );
      ...
      const studentInfo: StudentInfoDto = {
        ...
        date_of_birth: userProfile.student.date_of_birth
          ? formatDate(userProfile.student.date_of_birth)
          : '',
        display_picture:
          typeof userProfile.display_picture === 'string'
      ...
      const studentClass: MobileStudentClassDto = {
        ...
        section: this.extractSectionFromClassName(
          userProfile.student.current_class?.name || '',
        ),
      };
      ...
      const academicInfo: AcademicInfoDto = {
        ...
        performance_summary: {
          ...
          failed_assessments:
            (performanceData.totalAssessments || 0) -
            (performanceData.passedAssessments || 0),
          current_rank: performanceData.classPosition || 0,
          total_students: performanceData.totalStudentsInClass || 0,
          grade_point_average: this.calculateGPA(
      ...
      const settings: SettingsDto = {
        notifications: {
          ...
          announcement_notifications:
            userSettings?.announcement_notifications ?? false,
        },
        app_preferences: {
      ...
      const supportInfo: SupportInfoDto = {
        help_center: {
          ...
          last_updated: supportInfoData?.last_faq_update
            ? formatDate(supportInfoData.last_faq_update)
            : '2024-10-01',
          categories: supportInfoData?.faq_categories
            ? JSON.parse(supportInfoData.faq_categories as string)
            : ['General', 'Academic', 'Technical', 'Account'],
        },
        contact_options: {
          email_support:
            supportInfoData?.email_support ??
            userProfile.school?.school_email ??
            'support@school.edu',
          phone_support:
            supportInfoData?.phone_support ??
            userProfile.school?.school_phone ??
            '+1-800-SCHOOL',
        ...
        app_info: {
          ...
          last_updated: supportInfoData?.last_updated
            ? formatDate(supportInfoData.last_updated)
            : '2024-10-01',
          minimum_ios_version: supportInfoData?.minimum_ios_version ?? '13.0',
          minimum_android_version:
            supportInfoData?.minimum_android_version ?? '8.0',
        },
      };
      ...
      this.logger.log(
        colors.blue(
          `Student display picture: ${JSON.stringify(userProfile.display_picture)}`,
        ),
      );

      this.logger.log(
        colors.green(
          `Mobile student profile retrieved successfully for: ${user.email}`,
        ),
      );
      ...
      this.logger.error(
        colors.red(`Error fetching mobile student profile: ${error.message}`),
      );
  ...
  private async getStudentSubjects(
    ...
    try {
      ...
      this.logger.warn(
        colors.yellow(`Could not fetch student subjects: ${error.message}`),
      );
      return [];
    }
  }

  private async getStudentAttendancePercentage(
    userId: string,
    classId: string | null,
  ): Promise<number> {
    try {
      ...
      this.logger.warn(
        colors.yellow(
          `Could not fetch attendance percentage: ${error.message}`,
        ),
      );
      return 0;
    }
  ...
  private async getUserSettings(userId: string) {
    try {
      ...
      this.logger.warn(
        colors.yellow(`Could not fetch user settings: ${error.message}`),
      );
      return null;
    }
  }
  ...
  private async getSupportInfo(schoolId: string) {
    try {
      ...
      this.logger.warn(
        colors.yellow(`Could not fetch support info: ${error.message}`),
      );
      return null;
    }
  }

  private async getStudentAchievements(
    userId: string,
    academicSessionId: string,
  ) {
    try {
      ...
      this.logger.warn(
        colors.yellow(`Could not fetch student achievements: ${error.message}`),
      );
      return [];
    }
  }
