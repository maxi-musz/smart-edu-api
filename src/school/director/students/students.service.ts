...
export class StudentsService {
  ...
  private async calculatePerformanceMetrics(
    studentId: string,
    classId: string,
  ): Promise<Omit<StudentPerformance, 'position'>> {
    ...
    const cgpa = Number(
      (
        (performances.reduce(
          (sum, perf) => sum + perf.total_score / perf.max_score,
          0,
        ) /
          performances.length) *
        4
    ...
    const termAverage = Number(
      ((performances[0].total_score / performances[0].max_score) * 100).toFixed(
        2,
      ),
    );
    ...
    const improvementRate =
      performances.length > 1
        ? Number(
            (
              (performances[0].total_score / performances[0].max_score -
                performances[1].total_score / performances[1].max_score) *
              100
            ).toFixed(2),
          )
        : 0;
  ...
  private async calculateClassPosition(
    ...
    const termNumber =
      currentSession.term === AcademicTerm.first
        ? 1
        : currentSession.term === AcademicTerm.second
          ? 2
          : 3;
    ...
    const position =
      performances.findIndex((p) => p.student_id === studentId) + 1;
    return position || 0;
  }

  async fetchStudentsDashboard(
    schoolId: string,
    dto: FetchStudentsDashboardDto = {},
  ) {
    ...
    const currentSession = currentSessionResponse.success
    ...
    const studentsWithDetails: StudentWithDetails[] = await Promise.all(
      students.map(async (student) => {
        ...
        return {
          ...
          next_class_teacher: nextClass
            ? `${nextClass.teacher.first_name} ${nextClass.teacher.last_name}`
            : null,
          performance: {
            ...
            position: await this.calculateClassPosition(
              currentClass.id,
              student.id,
              currentSession,
            ),
    ...
    this.logger.log(
      colors.blue('Students dashboard data fetched successfully'),
    );
  ...
  async addStudentToClass(user: User, dto: AddStudentToClassDto) {
    this.logger.log(
      colors.cyan(
        `Adding student to class - Student: ${dto.student_id}, Class: ${dto.class_id}`,
      ),
    );
    ...
    try {
      ...
      if (!managedClass) {
        this.logger.log(
          colors.red(`Teacher is not managing class: ${dto.class_id}`),
        );
        return new ApiResponse(
          false,
          'You can only add students to classes you manage',
          null,
        );
      }
      ...
      if (existingEnrollment) {
        this.logger.log(
          colors.yellow(`Student already enrolled in class: ${dto.class_id}`),
        );
        return new ApiResponse(
          false,
      ...
      this.logger.error(
        colors.red(`Error adding student to class: ${error.message}`),
      );
  ...
  async getAvailableClasses(user: User) {
    this.logger.log(
      colors.cyan(`Fetching available classes for teacher: ${user.email}`),
    );
    ...
    try {
      ...
      this.logger.log(
        colors.green(`Found ${formattedClasses.length} available classes`),
      );

      return new ApiResponse(true, 'Available classes fetched successfully', {
        classes: formattedClasses,
        summary: {
          total_classes: formattedClasses.length,
          classes_with_teachers: formattedClasses.filter(
            (cls) => cls.class_teacher !== null,
          ).length,
          classes_without_teachers: formattedClasses.filter(
            (cls) => cls.class_teacher === null,
          ).length,
        },
      });
      ...
      this.logger.error(
        colors.red(`Error fetching available classes: ${error.message}`),
      );
  ...
  async enrollStudentToClass(user: User, dto: AddStudentToClassDto) {
    this.logger.log(
      colors.cyan(
        `Enrolling student to class - Student: ${dto.student_id}, Class: ${dto.class_id}`,
      ),
    );
    ...
    try {
      ...
      if (!managedClass) {
        ...
        return new ApiResponse(
          false,
          'You can only enroll students to classes you manage',
          null,
        );
      }
      ...
      this.logger.log(
        colors.green(
          `Student ${student.first_name} ${student.last_name} enrolled in class ${managedClass.name} successfully`,
        ),
      );
      ...
      this.logger.error(
        colors.red(`Error enrolling student to class: ${error.message}`),
      );
  ...
  async enrollNewStudent(user: User, dto: EnrollNewStudentDto) {
    this.logger.log(
      colors.blue(`Enrolling new student, Dto: ${JSON.stringify(dto)}`),
    );
    ...
    try {
      ...
      this.logger.log(
        colors.cyan(
          `🔍 Using user ID: ${userId} (user.sub: ${(user as any).sub}, user.id: ${user.id})`,
        ),
      );
      ...
      const debugClasses = availableClasses.map((c) => ({
        id: c.id,
        classId: c.classId,
        name: c.name,
      }));
      this.logger.log(
        colors.yellow(
          `all available classes for this school: ${JSON.stringify(debugClasses)}`,
        ),
      );
      ...
      if (dto.class_id) {
        this.logger.log(
          colors.cyan(
            `🔍 Checking class_id: ${dto.class_id} for school: ${fullUser.school_id}`,
          ),
        );
        ...
        if (!classExists) {
          this.logger.log(
            colors.red(
              `❌ Class not found! Searching for class_id: ${dto.class_id}`,
            ),
          );
          this.logger.log(
            colors.yellow(
              `Available class IDs: ${JSON.stringify(availableClasses.map((c) => c.id))}`,
            ),
          );
          console.log(
            colors.red(`Specified class not found or doesn't belong to school`),
          );
          return new ApiResponse(
            false,
        ...
        this.logger.log(
          colors.green(
            `✅ Class verified: ${classExists.name} (ID: ${classExists.id})`,
          ),
        );
      } else {
        this.logger.log(
          colors.yellow(
            `⚠️ No class specified - student will be enrolled without a class`,
          ),
        );
      }
      ...
      try {
        ...
        this.logger.error(
          colors.red(`❌ Failed to create student: ${error.message}`),
          error,
        );
        return new ApiResponse(
          false,
      ...
      this.logger.log(
        colors.green(
          `✅ New student enrolled successfully: ${dto.first_name} ${dto.last_name}`,
        ),
      );
  ...
  async updateStudent(studentId: string, dto: UpdateStudentDto, user: any) {
    ...
    this.logger.log(
      colors.cyan(`Updating student with ID: ${studentId} by user: ${userId}`),
    );
    ...
    try {
      ...
      this.logger.log(
        colors.cyan(`school admin user data: ${JSON.stringify(schoolAdmin)}`),
      );
      ...
      this.logger.log(
        colors.yellow(
          `🔍 Debug - Student lookup by ID (${studentId}): ${JSON.stringify(studentWithoutSchoolFilter)}`,
        ),
      );
      ...
      if (!studentWithoutSchoolFilter) {
        this.logger.log(
          colors.yellow(
            `🔍 Debug - Student not found by ID, trying user_id lookup...`,
          ),
        );
        ...
        this.logger.log(
          colors.yellow(
            `🔍 Debug - Student lookup by user_id (${studentId}): ${JSON.stringify(studentWithoutSchoolFilter)}`,
          ),
        );
      }
      ...
      if (!studentWithoutSchoolFilter) {
        this.logger.error(
          colors.red(
            `Student with ID/user_id ${studentId} does not exist in database`,
          ),
        );
      ...
      this.logger.log(
        colors.yellow(
          `🔍 Debug - Student's user record: ${JSON.stringify(studentUser)}`,
        ),
      );
      this.logger.log(
        colors.yellow(
          `🔍 Debug - Student record school_id: ${studentWithoutSchoolFilter.school_id}, Student user school_id: ${studentUser?.school_id}, Admin school_id: ${schoolAdmin.school_id}`,
        ),
      );

      // Verify school access - check both student record and user record school_id
      const studentSchoolId =
        studentUser?.school_id || studentWithoutSchoolFilter.school_id;

      if (studentSchoolId !== schoolAdmin.school_id) {
        this.logger.error(
          colors.red(
            `Student belongs to different school. Student school: ${studentSchoolId}, Admin school: ${schoolAdmin.school_id}`,
          ),
        );
        return new ApiResponse(
          false,
      ...
      if (
        studentWithoutSchoolFilter.school_id !== schoolAdmin.school_id &&
        studentUser?.school_id === schoolAdmin.school_id
      ) {
        this.logger.warn(
          colors.yellow(
            `⚠️ Data inconsistency: Student record has school_id ${studentWithoutSchoolFilter.school_id} but user has school_id ${studentUser.school_id}. Using user's school_id.`,
          ),
        );
      }
      ...
      if (!existingStudent) {
        this.logger.error(
          colors.red(`Student record not found: ${actualStudentId}`),
        );
        return new ApiResponse(false, 'Student not found', 404);
      }
      ...
      if (existingStudent.user.school_id !== schoolAdmin.school_id) {
        this.logger.error(
          colors.red(
            `Access denied: Student user belongs to different school. Student user school: ${existingStudent.user.school_id}, Admin school: ${schoolAdmin.school_id}`,
          ),
        );
        return new ApiResponse(
          false,
      ...
      if (dto.class_id && dto.class_id !== existingStudent.current_class_id) {
        ...
        if (!newClass) {
          ...
          return new ApiResponse(
            false,
            'New class not found or access denied',
            404,
          );
        }
      }
      ...
      if (dto.display_picture !== undefined)
        userUpdateData.display_picture = dto.display_picture;
      ...
      if (dto.date_of_birth !== undefined)
        studentUpdateData.date_of_birth = dto.date_of_birth
          ? new Date(dto.date_of_birth)
          : null;
      ...
      if (dto.guardian_phone !== undefined)
        studentUpdateData.guardian_phone = dto.guardian_phone;
      if (dto.guardian_email !== undefined)
        studentUpdateData.guardian_email = dto.guardian_email;
      if (dto.address !== undefined) studentUpdateData.address = dto.address;
      if (dto.emergency_contact !== undefined)
        studentUpdateData.emergency_contact = dto.emergency_contact;
      if (dto.blood_group !== undefined)
        studentUpdateData.blood_group = dto.blood_group;
      if (dto.medical_conditions !== undefined)
        studentUpdateData.medical_conditions = dto.medical_conditions;
      if (dto.allergies !== undefined)
        studentUpdateData.allergies = dto.allergies;
      if (dto.previous_school !== undefined)
        studentUpdateData.previous_school = dto.previous_school;
      if (dto.academic_level !== undefined)
        studentUpdateData.academic_level = dto.academic_level;
      if (dto.parent_id !== undefined)
        studentUpdateData.parent_id = dto.parent_id;
      if (dto.class_id !== undefined)
        studentUpdateData.current_class_id = dto.class_id;
      ...
      if (dto.class_id && dto.class_id !== existingStudent.current_class_id) {
        ...
        classTransferInfo = {
          from: existingStudent.current_class_id
            ? 'Previous Class'
            : 'No Class',
      ...
      this.logger.log(
        colors.green(`✅ Student updated successfully: ${studentId}`),
      );
