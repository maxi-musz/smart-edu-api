...
import {
  ...
  sendClassManagementEmail,
} from 'src/common/mailer/send-assignment-notifications';
...
export class TeachersService {
  ...
  private async cleanupOrphanedUsers(email: string) {
    try {
      ...
      if (orphanedUsers.length > 0) {
        this.logger.log(
          colors.yellow(
            `Found ${orphanedUsers.length} orphaned user(s) for email: ${email}`,
          ),
        );
        ...
        this.logger.log(
          colors.green(
            `✅ Cleaned up ${orphanedUsers.length} orphaned user(s) for email: ${email}`,
          ),
        );
      }
    } catch (error) {
      this.logger.error(
        colors.red(`❌ Failed to cleanup orphaned users for email: ${email}`),
        error,
      );
    }
  }
  ...
  async fetchClassesAndSubjects(dto: { school_id: string }) {
    this.logger.log(
      colors.cyan('Fetching classes and subjects for teacher creation...'),
    );
    ...
    try {
      ...
      this.logger.log(
        colors.green(
          `Total of ${classes.length} classes and ${subjects.length} subjects fetched successfully`,
        ),
      );
      ...
      this.logger.error(
        colors.red('Error fetching classes and subjects: '),
        error,
      );
      throw error;
    }
  ...
  async fetchTeachersDashboard(dto: FetchTeachersDashboardDto) {
    this.logger.log(
      colors.cyan('Fetching teacher dashboard for school director...'),
    );
    ...
    try {
      ...
      this.logger.log(
        colors.green(
          `Teachers dashboard data fetched successfully, total of ${totalTeachers} teachers`,
        ),
      );
      return ResponseHelper.success(
      ...
      console.log(
        colors.red('Error fetching teachers dashboard data: '),
        error,
      );
      throw error;
    }
  ...
  async enrollNewTeacher(dto: AddNewTeacherDto & { user: any }) {
    ...
    this.logger.log(
      colors.cyan(
        `Enrolling new teacher for school_id: ${loggedInUser.school_id}`,
      ),
    );
    ...
    try {
      ...
      try {
        ...
        if (subjectNames.length > 0 || classNames.length > 0) {
          ...
          this.logger.log(
            colors.green(
              `✅ Assignment notifications sent to teacher: ${email}`,
            ),
          );
        }
        ...
        try {
          ...
          this.logger.log(
            colors.green(
              `✅ Director notifications sent for new teacher: ${email}`,
            ),
          );
        } catch (directorEmailError) {
          this.logger.error(
            colors.red(
              `❌ Failed to send director notifications for new teacher: ${email}`,
            ),
            directorEmailError,
          );
        ...
        this.logger.error(
          colors.red(
            `❌ Failed to send assignment notifications to teacher: ${email}`,
          ),
          assignmentEmailError,
        );
      ...
      try {
        ...
        if (pushResult.success) {
          this.logger.log(
            colors.green(`✅ Welcome push notification sent to new teacher`),
          );
        } else {
          this.logger.warn(
            colors.yellow(
              `⚠️ Welcome push notification failed: ${pushResult.message}`,
            ),
          );
        }
      } catch (pushError) {
      ...
      this.logger.error(
        colors.red(`❌ Failed to create teacher: ${error.message}`),
        error,
      );
      return ResponseHelper.error(
  ...
  async updateTeacher(id: string, dto: UpdateTeacherDto, user: any) {
    try {
      ...
      if (!existingTeacher) {
        this.logger.error(
          colors.red(
            `Teacher not found or doesn't belong to school: ${fullUser.school_id}`,
          ),
        );
      ...
      if (dto.display_picture !== undefined)
        updateData.display_picture = dto.display_picture;
      ...
      if (dto.subjectsTeaching && Array.isArray(dto.subjectsTeaching)) {
        this.logger.log(
          colors.cyan(`Updating subject assignments for teacher: ${id}`),
        );
        ...
        if (subjectsToAdd.length > 0) {
          this.logger.log(
            colors.cyan(
              `Adding ${subjectsToAdd.length} new subject assignments`,
            ),
          );
        ...
        if (subjectsToRemove.length > 0) {
          this.logger.log(
            colors.cyan(
              `Removing ${subjectsToRemove.length} subject assignments`,
            ),
          );
      ...
      if (dto.classesManaging && Array.isArray(dto.classesManaging)) {
        this.logger.log(
          colors.cyan(`Updating class assignments for teacher: ${id}`),
        );
        ...
        if (classesToAdd.length > 0) {
          this.logger.log(
            colors.cyan(`Adding ${classesToAdd.length} new class assignments`),
          );
        ...
        if (classesToRemove.length > 0) {
          this.logger.log(
            colors.cyan(`Removing ${classesToRemove.length} class assignments`),
          );
      ...
      try {
        ...
        if (existingTeacher.subjectsTeaching.length > 0) {
          previousSubjectNames.push(
            ...existingTeacher.subjectsTeaching.map((ts) => ts.subject.name),
          );
        }

        if (existingTeacher.classesManaging.length > 0) {
          previousClassNames.push(
            ...existingTeacher.classesManaging.map((c) => c.name),
          );
        }

        if (
          newSubjectNames.length > 0 ||
          removedSubjectNames.length > 0 ||
          newClassNames.length > 0 ||
          removedClassNames.length > 0
        ) {
          await sendAssignmentNotifications(
            id,
          ...
          this.logger.log(
            colors.green(
              `✅ Assignment update notifications sent to teacher: ${updatedTeacher.email}`,
            ),
          );
        }
        ...
        try {
          ...
          this.logger.log(
            colors.green(
              `✅ Director notifications sent for teacher assignment changes: ${updatedTeacher.email}`,
            ),
          );
        } catch (directorEmailError) {
          this.logger.error(
            colors.red(
              `❌ Failed to send director notifications for teacher assignment changes: ${updatedTeacher.email}`,
            ),
            directorEmailError,
          );
          // Don't fail the entire operation if director email fails
        ...
        this.logger.error(
          colors.red(
            `❌ Failed to send assignment update notifications to teacher: ${updatedTeacher.email}`,
          ),
          assignmentEmailError,
        );
        // Don't fail the entire operation if assignment email fails
  ...
  async assignSubjects(teacherId: string, subjectIds: string[]) {
    try {
      ...
      try {
        ...
        this.logger.log(
          colors.green(
            `✅ Teaching role notification sent to teacher: ${teacher.email}`,
          ),
        );
      } catch (emailError) {
        this.logger.error(
          colors.red(
            `❌ Failed to send teaching role notification to teacher: ${teacher.email}`,
          ),
          emailError,
        );
  ...
  async assignClass(teacherId: string, classId: string) {
    try {
      ...
      try {
        ...
        this.logger.log(
          colors.green(
            `✅ Class management notification sent to teacher: ${teacher.email}`,
          ),
        );
      } catch (emailError) {
        this.logger.error(
          colors.red(
            `❌ Failed to send class management notification to teacher: ${teacher.email}`,
          ),
          emailError,
        );
        // Don't fail the operation if email fails
  ...
  async fetchTeacherClassesAndSubjects(teacherId: string, user: User) {
    this.logger.log(
      colors.cyan(`Fetching classes and subjects for teacher: ${teacherId}`),
    );
    ...
    try {
      ...
      if (!teacher) {
        this.logger.error(
          colors.red(
            `Teacher not found or doesn't belong to school: ${user.school_id}`,
          ),
        );
        return ResponseHelper.error('Teacher not found or access denied', 404);
      }
      ...
      this.logger.log(
        colors.green(
          `Successfully fetched data for teacher: ${teacher.first_name} ${teacher.last_name}`,
        ),
      );
      ...
      this.logger.error(
        colors.red(
          `Error fetching teacher classes and subjects: ${error.message}`,
        ),
      );
      return ResponseHelper.error(
