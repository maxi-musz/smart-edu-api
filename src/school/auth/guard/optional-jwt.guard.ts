...
export class OptionalJwtGuard extends AuthGuard('jwt1') {
  ...
  canActivate(context: ExecutionContext) {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      console.log(
        colors.gray(
          'Optional JWT Guard - No valid auth header, proceeding without user',
        ),
      );
  ...
  handleRequest(err: any, user: any, info: any) {
    ...
    if (err || !user) {
      console.log(
        colors.yellow(
          'Optional JWT Guard - Token invalid or expired, proceeding without user',
        ),
      );
      return null;
    }
    ...
    console.log(
      colors.green(`Optional JWT Guard - User authenticated: ${user.sub}`),
    );
    return user;
  }
}
