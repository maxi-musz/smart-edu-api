...
export class TemEndpointService {
  ...
  async bulkUpdatePasswordsAndGenerateReport(
    ...
    const persRows: Record<string, any>[] = XLSX.utils.sheet_to_json(
      persSheet,
      { defval: '' },
    );
    ...
    const persFileNumberKey = persKeys.find(
      (k) => k.includes('file') && k.includes('number'),
    );
    ...
    this.logger.log(
      `Personal-email map built with ${personalEmailMap.size} entries`,
    );
    ...
    const mainRows: Record<string, any>[] = XLSX.utils.sheet_to_json(
      mainSheet,
      { defval: '' },
    );
    ...
    const mainFileNumberKey = mainKeys.find(
      (k) => k.includes('file') && k.includes('number'),
    );
    const smartEduEmailKey = mainKeys.find(
      (k) =>
        (k.includes('smart') && k.includes('email')) || k === 'smart_edu_email',
    );
    ...
    for (let i = 0; i < normalizedMainRows.length; i++) {
      ...
      if (!smartEduEmail || !password) {
        this.logger.warn(
          `[SKIP] Row ${i + 1} missing smart_edu_email or password`,
        );
        failedCount++;
        results.push({
          sn: i + 1,
  ...
  async sendCredentialsEmail(
    ...
    const personalEmailKey = keys.find(
      (k) => k.includes('personal') && k.includes('email'),
    );
    ...
    this.logger.log(
      `Detected columns — smart_edu_email: "${smartEduEmailKey}", personal_email: "${personalEmailKey}", password: "${passwordKey}"`,
    );
    this.logger.log(
      `Sending credentials to ${normalizedRows.length} recipient(s)...`,
    );
    ...
    for (const row of normalizedRows) {
      ...
      if (!personalEmail || !smartEduEmail || !password) {
        this.logger.warn(
          `[SKIP] Missing data — personalEmail: "${personalEmail || 'EMPTY'}", smartEduEmail: "${smartEduEmail || 'EMPTY'}"`,
        );
   ...
   */
  private buildCredentialsEmailHtml(
    username: string,
    password: string,
  ): string {
    return `
