...
export class TemEndpointController {
    ...
    async bulkUpdatePasswords(
        @UploadedFiles() files: { file?: Express.Multer.File[]; personalFile?: Express.Multer.File[] },
        @Res() res: Response,
    ) {
        if (!files?.file?.[0] || !files?.personalFile?.[0]) {
            throw new BadRequestException({
                success: false,
                message: 'Two files are required: "file" (smart-edu-email + password Excel) and "personalFile" (file-number + personal-email Excel).',
                statusCode: 400,
            });
        }
        ...
        const { excelBuffer, summary } = await this.temEndpointService.bulkUpdatePasswordsAndGenerateReport(
            files.file[0].buffer,
    ...
    async sendCredentialsEmail(
        @UploadedFiles() files: { file?: Express.Multer.File[]; guide?: Express.Multer.File[] },
    ) {
        if (!files?.file?.[0] || !files?.guide?.[0]) {
            throw new BadRequestException({
                success: false,
                message: 'Two files are required: "file" (the credentials Excel) and "guide" (the PDF instruction guide).',
                statusCode: 400,
            });
        }
        ...
        if (!validExcelTypes.includes(excelFile.mimetype)) {
            throw new BadRequestException({
                success: false,
                message: 'Invalid file type for "file". Please upload an .xlsx or .xls file.',
                statusCode: 400,
            });
        }
