...
export class StudentsService {
  ...
  /**
   * Get student results for a specific session/term
   * @param user - User object with sub and email
   * @param sessionId - Optional academic session ID
   * @param termId - Optional term ID (not used, term is part of session)
   */
  async getStudentResults(user: any, sessionId?: string, termId?: string) {
    this.logger.log(colors.cyan(`Fetching student results for: ${user.email}`));
    ...
    try {
      ...
      // Get academic session (use provided sessionId or get current)
      let academicSession: any;
      if (sessionId) {
