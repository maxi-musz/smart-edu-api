...
export const UpdateProfilePictureDocs = {
  operation: ApiOperation({
    ...
    description:
      'Upload and update user profile picture. Works for all roles (student, teacher, director). The new picture will replace the existing one. If the update fails, the uploaded file will be automatically deleted from storage.',
  }),
  bearerAuth: ApiBearerAuth('JWT-auth'),
          ...
          properties: {
            display_picture: {
              ...
              properties: {
                url: {
                  type: 'string',
                  example: 'https://storage.example.com/profile-pictures/...',
                },
                key: {
                  type: 'string',
                  example:
                    'profile-pictures/schools/.../profile_1234567890.jpg',
                },
                bucket: {
                  type: 'string',
                  example: 'my-bucket',
  ...
  response400: ApiResponse({
    status: 400,
    description:
      'Bad request - Invalid file type, file too large, or upload failed',
  }),
  response401: ApiResponse({
    status: 401,
    description: 'Unauthorized - Invalid or missing JWT token',
